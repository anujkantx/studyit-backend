...
SessionLocal = sessionmaker(       
    ...
    expire_on_commit=False, # This option is used to specify that the objects in the session should not be expired after a commit. By default, SQLAlchemy expires all objects in the session after a commit, which means that they will be reloaded from the database the next time they are accessed. Setting expire_on_commit to False prevents this behavior, allowing us to continue using the objects in the session without having to reload them from the database after a commit.
)
